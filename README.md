...
<!--
  ...
  - shields.io: followers/stars badges — pulls live GitHub API data
    through Shields' own CDN, more reliable than the trophy generators
    (every github-profile-trophy instance/mirror tried was unreliable,
    so dropped that approach entirely)
  - the isometric calendar is NOT a live third-party service — it's
    generated by lowlighter/metrics running as a GitHub Action in your
    own repo (metrics.yml). It only renders once that Action has been
    added to the repo, given a token, and run at least once.
-->
